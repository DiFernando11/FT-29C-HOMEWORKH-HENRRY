...
const author = document.querySelector("#createdBy");
author.innerHTML = `${author.textContent} Diego Apolo`;
...
function buildToDos(toDos) {
  console.log(toDos);
  let arrayToDo = toDos.map((todo, index) => {
...
function displayToDos() {
  // se encarga que los todos se vean en pantalla
  // Tu código acá:
  //paso1
  ...
  //paso3
  resultValueBuilToDos = buildToDos(toDoItems);
  //paso 4
  resultValueBuilToDos.forEach((element) => {
    return toDoContainer.appendChild(element);
  });
  // for (let i = 0; i < resultValueBuilToDos.length; i++) {
  //   toDoContainer.appendChild(resultValueBuilToDos[i]);
  // }
}
...
function addToDo() {
  let nodeInput = document.getElementById("toDoInput");
  let newTodo = new ToDo(nodeInput.value);
  //console.log(newTodo);
